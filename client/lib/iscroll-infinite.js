...
var utils = (function () {
	...
	me.momentum = function (current, start, time, lowerMargin, wrapperSize, deceleration) {
		var distance = current - start,
			speed = Math.abs(distance) * 1.8 / time,	//phuongnq change: increase scroll speed
			destination,
			duration;
...
// INSERT POINT: _wheel
	...
	_initInfinite: function () {
		...
		//phuongnq reset elements._top
		for (var i = 0; i < this.infiniteElements.length; i++) {
	    this.infiniteElements[i]._top = undefined;
	  }
		...
		this.on('refresh', function () {
			...
			//phuongnq fix when infiniteLength too small
			this.infiniteUpperBufferSize = Math.max(0, Math.floor((this.infiniteLength - elementsPerPage) / 2));
